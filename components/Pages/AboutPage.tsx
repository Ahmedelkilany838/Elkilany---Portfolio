                ...
                >
                    <div className="w-full flex flex-wrap md:grid md:grid-cols-3 items-end justify-between gap-y-6 md:gap-y-0">
                        ...
                        <div className="flex justify-center items-end gap-2 md:gap-4 order-3 md:order-2 w-full md:w-auto pb-0 md:pb-1 mt-4 md:mt-0">
                            <SocialCircle href="https://www.instagram.com/ahmed.el_kilany/">
                            ...
                            <SocialCircle href="https://www.linkedin.com/in/ahmed-el-kilany-2262b3303/">
                            ...
                            <SocialCircle href="https://www.facebook.com/ahmedg.elkelany">
                            ...
                            <SocialCircle href="https://dribbble.com/ahmed-elkilany">
                    ...
                    <div className="flex items-center gap-4 mb-6 md:mb-10 text-[#777] text-xs font-mono uppercase tracking-[0.1em]">
                        ...
                        <div className="flex overflow-hidden whitespace-nowrap opacity-80 w-[200px] md:w-[300px]" style={{ maskImage: 'linear-gradient(to right, black 60%, transparent)' }} aria-hidden="true">
                            <motion.div
                                ...
                                className="flex"
                            >
                                <span className="mr-4">WHO I AM — WHO I AM — WHO I AM — </span>
                                <span className="mr-4">WHO I AM — WHO I AM — WHO I AM — </span>
                            </motion.div>
                        </div>
                    </div>
...
function ParallaxImageBreak({ parallaxImage }: { parallaxImage: string }) {
    const containerRef = useRef<HTMLElement>(null);
...
// ─────────────────────────────────────────────────────────────────────────────
// ABOUT HERO WRAPPER — owns the useTransform hook (Rules of Hooks compliant)
// ─────────────────────────────────────────────────────────────────────────────
...
function AboutHeroWrapper({ scrollY, heroImage }: AboutHeroWrapperProps) {
    ...
    return (
        ...
        >
            <HeroSection scrollY={scrollY} heroImage={heroImage} />
        </motion.div>
    );
}

// ─────────────────────────────────────────────────────────────────────────────
// MAIN EXPORT — Wires all sections together dynamically
// ─────────────────────────────────────────────────────────────────────────────
export default function AboutPage() {
    const { scrollY } = useScroll();
    const config = DEFAULT_CONFIG;

    const aboutSections = config.pages?.about?.sections || [];
    const heroSec = aboutSections.find(s => s.id === 'aboutHero');
    const otherSecs = aboutSections.filter(s => s.id !== 'aboutHero');
    ...
    const renderSection = (sec: any) => {
        ...
        switch (sec.id) {
            case 'aboutHero':
                return <AboutHeroWrapper key={sec.id} scrollY={scrollY} heroImage={config.images.aboutHeroImage} />;
            case 'manifesto': return <ManifestoSection key={sec.id} />;
            case 'stats': return <Stats key={sec.id} className="-mt-8 md:-mt-16" />;
            case 'teamsBrands': return (
                <div key={sec.id} className="w-full overflow-hidden" style={{ backgroundColor: 'var(--site-page-bg, #050505)' }}>
                    <BrandsMarquee />
                </div>
            );
            case 'timeline': return <TimelineSection key={sec.id} />;
            case 'parallax': return <ParallaxImageBreak key={sec.id} parallaxImage={config.images.parallaxBreakImage} />;
            case 'methodology': return <MethodologySection key={sec.id} />;
            case 'capabilities': return <CapabilitiesSection key={sec.id} />;
            case 'faq': return <FAQ key={sec.id} />;
            case 'cta': return <FinalCTA key={sec.id} />;
            case 'contact': return <Contact key={sec.id} />;
            default: return null;
        }
    };

    return (
        <div className="relative w-full" style={{ backgroundColor: 'var(--site-page-bg, #050505)' }}>
            {/* 1. Identity Hero (Fixed/Sticky) */}
            {renderSection(heroSec || { id: 'aboutHero', visible: true })}

            {/* 2. Scrollable Sections (Cover the Hero) */}
            <div className="relative z-20 mt-[50vh]" style={{ backgroundColor: 'var(--site-page-bg, #050505)' }}>
                {otherSecs.map(sec => {
